...
1. Read `~/.claude/craftsman/directives/index.md` and/or `~/.claude/craftsman/protocols/index.md` per `$ARGUMENTS` (both, if no argument is given).
...
- Read-only. The index files are the single source of truth for current enabled/disabled state — see `EXECUTION.md`, "Loading directives".
