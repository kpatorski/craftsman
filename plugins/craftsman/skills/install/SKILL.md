...
4. Validate syntax (MANAGEMENT.md) — the directive/protocol contract, or the bundle contract if `$ARGUMENTS` is a
...
6. Write the file(s): under `~/.claude/craftsman/directives/<id>/` or `protocols/<id>/` for a standalone entry, or
   ...
   directive, or always-Enabled for a protocol or bundle), and add or update the `Sources` table entry.
