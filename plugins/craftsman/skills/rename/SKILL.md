...
3. Rename the directory, update the `id` field in its own frontmatter, and update every `composes` / `steps` / `uses` /
   `overrides` entry and every body link across both `directives/` and `protocols/` that pointed at the old id.
